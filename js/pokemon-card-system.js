...
const PokeballSystem = {
    ...
    /**
     * 计算捕捉成功率
     * @param {Object} ball - 精灵球类型
     * @param {Object} enemy - 敌人数据
     * @returns {number} 成功率 (0-1)
     */
    calculateCatchRate(ball, enemy) {
        if (!enemy) return 0;
        
        const ballData = this.BALL_TYPES[ball.type] || this.BALL_TYPES.pokeball;
        let catchRate = ballData.baseCatchRate;
        
        // 根据剩余HP调整
        const hpPercent = enemy.currentHp / enemy.hp;
        const hpMultiplier = 1 + (1 - hpPercent) * 1.5; // HP越低，倍率越高
        catchRate *= hpMultiplier;
    ...
    attemptCapture(ballType) {
        ...
        if (!enemy) {
            return { success: false, message: '没有可以捕捉的目标！' };
        }
        
        // 检查是否已有6只宝可梦
        if (!PokemonCardSystem.canAddPokemon()) {
            return { success: false, message: '宝可梦牌组已满！' };
        }
        
        const ball = this.BALL_TYPES[ballType] || this.BALL_TYPES.pokeball;
        const catchRate = this.calculateCatchRate({ type: ballType }, enemy);
        
        // 随机判定
        const roll = Math.random();
        const success = roll < catchRate;
        
        console.log(`捕捉判定: 成功率=${(catchRate*100).toFixed(1)}%, 骰点=${(roll*100).toFixed(1)}%, 结果=${success?'成功':'失败'}`);
        
        if (success) {
            // 创建捕捉到的宝可梦数据
            const capturedPokemon = {
                id: `captured-${enemy.id}-${Date.now()}`,
                name: enemy.name,
                types: enemy.types || ['normal'],
                hp: Math.floor(enemy.hp * 0.8), // 捕捉后HP略低
                maxHp: Math.floor(enemy.hp * 0.8),
                sprite: enemy.sprite,
                skills: this.generateSkillsFromEnemy(enemy),
                captured: true
            };
            
            // 添加到宝可梦牌组
            PokemonCardSystem.addPokemon(capturedPokemon);
            
            // 战斗胜利
            return { 
                success: true, 
                pokemon: capturedPokemon,
                message: `成功捕捉了 ${enemy.name}！`
            };
        } else {
            return { 
                success: false, 
                message: `${enemy.name} 逃脱了！`
            };
        }
    },
     ...
     * 从敌人数据生成技能
     */
    generateSkillsFromEnemy(enemy) {
        const skills = [];
        
        if (enemy.moves && enemy.moves.length > 0) {
            // 从敌人招式中选择1-2个作为技能
            const selectedMoves = enemy.moves.slice(0, Math.min(2, enemy.moves.length));
            ...
            for (const move of selectedMoves) {
                skills.push({
                    ...
                    block: move.block || 0,
                    energy: move.damage ? Math.ceil(move.damage / 5) : 1,
    ...
    canCapture() {
        ...
        if (enemy.hp <= 0) return false; // 已击败的不能捕捉
        if (!PokemonCardSystem.canAddPokemon()) return false;
        return true;
    },
    ...
    getCaptureHint() {
        ...
        const catchRate = this.calculateCatchRate({ type: 'pokeball' }, enemy);
        const hpPercent = Math.floor((enemy.currentHp / enemy.hp) * 100);

        return {
            hpPercent: hpPercent,
            catchRate: Math.floor(catchRate * 100),
